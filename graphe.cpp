...
void graphe::initaliserAdj()
{
    adj = new std::vector<std::pair<int,int>> [2*m_aretes.size()];

    for(auto a: m_aretes)
    {
        float w=a->getCout2();
        adj[a->getId_sommet1()].push_back(std::make_pair(a->getId_sommet2(), w));
        adj[a->getId_sommet2()].push_back(std::make_pair(a->getId_sommet1(), w));
    }
            ...
            for(auto a : m_aretes)
            {
                if(a->getBool() != 1)
                {
                    ...
                    if(( num == sommets[0])||(num == sommets[1]))  /// Si l'id du sommet initial correspond a une arrete
                        ...
                        {
                            aretes.push_back(a);
                            a->utiliser(1); /// "marquer" l'arete utilisée pour pas qu'on la reprenne
                        }
                }
            }
...
std::vector<graphe>graphe::multipleBruteForce()
{
    int i;
    int b;
    i=m_sommets.size()-1;
...
/*
    for(auto a : ParetoND )
    {
        std::cout << " VERT | COUT 1 : " << a.getPoidsTotal1() << " COUT 1 : " << a.getPoidsTotal2() << std::endl;
...
int graphe::dijkstra(int source) //Algorithm for SSSP
{
    this->initaliserAdj();
